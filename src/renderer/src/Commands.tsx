...
type Entry = { kind: 'slash' | 'cli' | 'bullpen'; cmd: string; desc: string }
...
const GROUPS: Group[] = [
  {
    ...
    entries: [
      ...
      { kind: 'slash', cmd: '/resume', desc: 'Pick or search a past session to continue.' },
      { kind: 'slash', cmd: '/compact', desc: 'Summarise the conversation so far to free context.' },
      { kind: 'cli', cmd: 'claude -c', desc: 'Continue the most recent session in this directory.' },
      { kind: 'cli', cmd: 'claude -r', desc: 'Resume: pick or search a past session.' }
    ]
  },
  {
    ...
    entries: [
      ...
      { kind: 'slash', cmd: '/init', desc: 'Scan the repo and generate a CLAUDE.md of its conventions.' }
    ]
  },
  {
    title: 'Bullpen',
